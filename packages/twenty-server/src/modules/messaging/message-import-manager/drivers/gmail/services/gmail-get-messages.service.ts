...
export class GmailGetMessagesService {
  ...
  async getMessages(
    ...
    const customLabelIds = (messageChannel.messageFolders ?? [])
      .filter(
        (folder) =>
          folder.isSynced &&
          isNonEmptyString(folder.externalId) &&
          isGmailCustomLabel(folder.externalId),
      )
      ...
      threadIds.map((threadId) =>
        batchedGmailClient.users.threads
          ...
          .then((response) => {
            ...
            const threadHasCustomLabel = threadMessages.some((threadMessage) =>
              (threadMessage.labelIds ?? []).some((labelId) =>
                customLabelIds.includes(labelId),
              ),
            );

            if (!threadHasCustomLabel) {
              return;
            }
